...
int main(int argc, char* argv[])
{
    ...
    int newArgc = static_cast<int>(newArgv.size())+1;
    int audioCount = (newArgc - 1) / 3;

    // create audioFiles array
    struct wavFile0* audioFiles = new wavFile0[audioCount];
    //std::for_each(audioFiles, audioFiles + audioCount, [](struct wavFile0& w) {
    //    w.fileName = (char*)("");
    //    w.startTime = 0.0f;
    //    w.volume = 1;
    //    });
    ...
    for (int i = 0; i < audioCount; i++)
    {
        ...
        for (channel = 0; channel < numChannels; channel++)
        {
            ...
            {
                float currentSample = audio.samples[channel][i];
