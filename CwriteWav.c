...
int main() {
    char filename[30] = "result.wav";
    struct WAVData* audio_data = audioread(filename);
    if (audio_data == NULL) {
        perror("文件打开失败!");
        return 1;
    }
    printf("音频格式: %hu\n", audio_data->header.audioFormat);
    printf("声道数: %hu\n", audio_data->header.numChannels);
    printf("采样率: %hu\n", audio_data->header.sampleRate);
    printf("采样深度: %hu\n", audio_data->header.bitsPerSample);
    printf("音频数据长度: %u\n", audio_data->header.subchunk2Size);
    printf("音频数据: %p\n", audio_data->sample);

    
    char outfilename[30] = "../test.wav";
